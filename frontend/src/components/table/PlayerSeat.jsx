  ...
  return (
    <div className={clsx("flex gap-2", positionStyles[position])}>
      ...
      <div className={clsx(
        ...
        {/* Turn indicator */}
        {isTurn && (
          <span className="text-yellow-400 text-xs font-bold animate-pulse">▶ TURN</span>
        )}
      </div>
      ...
      {trickCard !== undefined && (
        <div className="relative">
          {trickCard ? (
            ...
            <div className="w-10 h-14 rounded border-dashed border border-slate-700 opacity-30" />
          )}
        </div>
      )}
    </div>
